  ...
  return (
    <View>
      ...
      <View style={[styles.topBar, { marginTop: TOP_BAR_TOP_SPACING }]}>
        {/* Left Icon (Scanner or Back) */}
        {(showScannerIcon || showBackButton) && (
          <TouchableOpacity 
            style={styles.iconLeft} 
            onPress={showBackButton ? onBackPress : (onScannerPress || (() => router.replace('/connectivity/qr-scanner')))}
          >
            {showBackButton ? (
              <Ionicons name="arrow-back" size={ICON_SIZE} color="#222" />
            ) : (
